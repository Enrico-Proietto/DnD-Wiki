...
async function loadCharacter() {
  try {
    const characterName = 'Willump Shinra';
    ...
    characterData = data;
    const willump = Object.values(data)[0][0];
...
function renderAbilities() {
  const willump = characterData.willumpShinra[0];
  const section = document.getElementById('abilitiesSection');
  const { abilityScores, skills } = willump;
...
function renderSpells() {
  ...
  let slotBlock = renderSpellSlots(willump);
...
function renderClassAbilities() {
  ...
  const classAbilities = willump.classAbilities;
...
function renderRaceAbilities() {
  ...
  const raceAbilities = willump.raceAbilities;
